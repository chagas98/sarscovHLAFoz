#!/bin/bash -ue
mkdir -p BE.10

cd BE.10

# Run Minimap2 to align the input file to the reference file
minimap2 -a ../NC_045512.2.fasta ../BE.10.fasta --secondary=no -N 0 > BE.10.sam

# Sort the aligned SAM file and save as BAM file
samtools sort BE.10.sam -o BE.10.bam

# Print the sorting status of the BAM file
samtools stats BE.10.bam | grep "is sorted:"

# Create an index file for the sorted BAM file
samtools index BE.10.bam  BE.10.bam.bai
